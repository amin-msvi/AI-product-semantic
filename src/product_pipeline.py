...
class AIProductPipeline:
    ...
    def _load_input_data(
        ...
        return {
            ...
            "queries": self.data_loader.load_data(queries_json, "json").get(
                "queries", []
            ),
        }
    ...
    def save_results(self, results: Dict, output_dir: str) -> None:
        ...
        self.data_loader.save_json(
            results["enriched_products"], output_path / "enriched_products.json"
        )

        # Save knowledge graph
        self.data_loader.save_json(
            results["knowledge_graph"], output_path / "knowledge_graph.json"
        )

        # Save query results
        self.data_loader.save_json(
            results["query_results"], output_path / "query_results.json"
        )
