...
class KnowledgeGraphBuilder:
    ...
    def build_graph(self, products: List[Dict]) -> Dict:
        ...
        self.logger.info(
            f"Built knowledge graph with {len(graph['products'])} products "
    ...
    def _create_relationships(self, product: Dict) -> List[Dict]:
        ...
        for intent in product.get("intents", []):
            relationships.append(
                {"type": "serves_intent", "source": product_id, "target": intent}
            )
        ...
        if category:
            relationships.append(
                {"type": "belongs_to", "source": product_id, "target": category}
            )
        return relationships
