...
class SchemaValidator:
    ...
    def validate_product(self, product, schema):
        ...
        if errors:
            self.logger.warning(
                f"Product {product_id} has {len(errors)} validation errors"
            )
        else:
    ...
    def validate_batch(self, products, schema):
        ...
        self.logger.info(
            f"Validated {len(products)} products. "
        ...
        for field_name, rule in schema.get("required_fields", {}).items():
            if field_name in product:
                field_errors = self._validate_field_rule(
                    product[field_name], rule, field_name
                )
    ...
    def _validate_field_rule(self, value: Any, rule, field_name):
        ...
        if "string" in rule_lower:
            if not isinstance(value, str):
                ...
                if max_length and len(value) > max_length:
                    errors.append(
                        f"Field '{field_name}' exceeds max length of {max_length} characters"
                    )
        ...
        # Float/numeric validation
            ...
            if not isinstance(value, (int, float)):
                errors.append(
                    f"Field '{field_name}' should be numeric, got {type(value).__name__}"
                )
